...
```{r testHallmark}
hallmarkRes <- fgsea(hallmark, idRankedByT, nperm=np, nproc = 12) %>%
	as_tibble() %>%
...
```{r testC2}
c2Res <- fgsea(c2, idRankedByT, nperm=np, minSize = 15, maxSize = 500, nproc = 12) %>%
	as_tibble() %>%
	dplyr::rename(FDR = padj) %>%
	mutate(adjP = p.adjust(pval, "bonferroni")) %>%
	dplyr::select(pathway, pval, FDR, adjP, everything()) %>%
...
```{r testC5}
c5Res <- fgsea(c5, idRankedByT, nperm=np, minSize = 15, maxSize = 200, nproc = 12) %>%
	as_tibble() %>%
	dplyr::rename(FDR = padj) %>%
	mutate(adjP = p.adjust(pval, "bonferroni")) %>%
	dplyr::select(pathway, pval, FDR, adjP, everything()) %>%
...
```{r testC7}
c7Res <- fgsea(c7, idRankedByT, nperm=np, minSize = 15, maxSize = 500, nproc = 12) %>%
	as_tibble() %>%
	dplyr::rename(FDR = padj) %>%
	mutate(adjP = p.adjust(pval, "bonferroni")) %>%
	dplyr::select(pathway, pval, FDR, adjP, everything()) %>%
