...
rawFqc %>%
	getModule("Sequence_Duplication") %>%
...
Instead of testing $H_0: \log FC = 0$ which tests for the point value zero, the null hypothesis was defined as being within the range [$-\tau, \tau$], with $\tau$ set at the default value of $\log_2 `r tau`$.
This additionally saves filtering any detected DE genes based on fold-change, as is common practice in many workflows.
...
```{r plotCPM, fig.cap=paste0("*Expression values for genes to an FDR of ", percent(thresh), ". Genes are shown in order of significance. Jitter has been added to the x-axis*"), fig.width=8, fig.height=8}
voomData$E %>%
...
treatFit$genes %<>%
	...
	dplyr::select(location, gene_name, gene_biotype, gene_id) %>%
	column_to_rownames("gene_id") 
```
...
treatFit %>%
	Glimma::glMDPlot(
		counts = geneDGE$counts %>% floor %>% cpm(),
		groups = geneDGE$samples$Group,
		status = decideTests(., p.value = thresh)[,2],
		main = "Diabetic Vs Non-Diabetic Fractures",
		xlab = "Average Expression (logCPM)",
		ylab = "logFC",
		side.ylab = "Counts Per Million (CPM)",
		sample.cols = geneDGE$samples$Group %>% 
			as.factor() %>%
			as.integer(),
		jitter = 10
)
...
Genes were ranked by t-statistic, and all results were permuted `r pander(comma(np))` times for greater precision in results. 
...
```{r testHallmark}
hallmarkRes <- fgsea(hallmark, idRankedByT, nperm=np, nproc = nCores) %>%
	as_tibble() %>%
...
hallmarkRes %>%
	...
	mutate(sizeAtMax = vapply(leadingEdge, length, integer(1)),
		   Genes = vapply(leadingEdge, function(x){
		   	results %>%
	...
	dplyr::select(Pathway = pathway, NES, `$p$` = pval, `$p_{adj}$` = adjP, size, sizeAtMax, Genes) %>%
	pander(split.tables = Inf,
		   caption = paste("Top", nrow(.), "enriched Hallmark gene-sets using a Bonferroni-adjusted p-value < 0.01 for stringency.",
...
```{r plotHallmark_1, fig.height=4, fig.width=12, fig.cap = "*Enrichment score for the most highly ranked hallmark geneset*"}
tempSet <- hallmarkRes$pathway[1]
a <- plotEnrichment(hallmark[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = str_remove(tempSet, "HALLMARK_"))
tempSet <- hallmarkRes$pathway[2]
b <- plotEnrichment(hallmark[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = str_remove(tempSet, "HALLMARK_"))
vpA <- viewport(0.25, 0.5, 0.5, 1)
vpB <- viewport(0.75, 0.5, 0.5, 1)
grid.newpage()
print(a, vp = vpA)
print(b, vp = vpB)
```
...
```{r testC2}
c2Res <- fgsea(c2, idRankedByT, nperm=np, minSize = 15, maxSize = 500, nproc = nCores) %>%
	as_tibble() %>%
	dplyr::rename(FDR = padj) %>%
	mutate(adjP = p.adjust(pval, "bonferroni")) %>%
	dplyr::select(pathway, pval, FDR, adjP, everything()) %>%
...
c2Res %>%
	...
	dplyr::select(Pathway = pathway, NES, `$p$` = pval, `$p_{adj}$` = adjP, size, sizeAtMax, Genes) %>%
	pander(split.tables = Inf,
		   caption = paste("Top", nrow(.), "enriched C2 gene-sets using a Bonferroni-adjusted p-value < 0.01 for stringency.",
		   				"The normalised enrichment score indicates which end of the ranked list appeared to be enriched, with negative values indicating the down regulated genes.",
		   				"The number of genes in the gene-set are also indicated, as are the number of genes at the point of maximum enrichment.",
		   				"The most highly ranked genes before the point of the maximal enrichment are given in the final column."),
		   style = "rmarkdown",
...
```{r plotC2_1, fig.height=4, fig.width=12, fig.cap = "*Enrichment score for the most highly ranked C2 genesets*"}
tempSet <- c2Res$pathway[1]
a <- plotEnrichment(c2[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = tempSet)
tempSet <- c2Res$pathway[2]
b <- plotEnrichment(c2[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = tempSet)
vpA <- viewport(0.25, 0.5, 0.5, 1)
vpB <- viewport(0.75, 0.5, 0.5, 1)
...
```{r testC5}
c5Res <- fgsea(c5, idRankedByT, nperm=np, minSize = 15, maxSize = 200, nproc = nCores) %>%
	as_tibble() %>%
	dplyr::rename(FDR = padj) %>%
	mutate(adjP = p.adjust(pval, "bonferroni")) %>%
	dplyr::select(pathway, pval, FDR, adjP, everything()) %>%
...
c5Res %>%
	...
	dplyr::select(Pathway = pathway, NES, `$p$` = pval, `$p_{adj}$` = adjP, size, sizeAtMax, Genes) %>%
	pander(split.tables = Inf,
		   caption = paste("Top", nrow(.), "enriched C5 gene-sets using a Bonferroni-adjusted p-value < 0.01 for stringency.",
		   				"The normalised enrichment score indicates which end of the ranked list appeared to be enriched, with negative values indicating the down regulated genes.",
		   				"The number of genes in the gene-set are also indicated, as are the number of genes at the point of maximum enrichment.",
		   				"The most highly ranked genes before the point of the maximal enrichment are given in the final column."),
		   style = "rmarkdown",
...
```{r plotC5_1, fig.height=4, fig.width=12, fig.cap = "*Enrichment score for the most highly ranked C5 genesets*"}
tempSet <- c5Res$pathway[1]
a <- plotEnrichment(c5[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = str_remove(tempSet, "GO_"))
tempSet <- c5Res$pathway[2]
b <- plotEnrichment(c5[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = str_remove(tempSet, "GO_"))
vpA <- viewport(0.25, 0.5, 0.5, 1)
vpB <- viewport(0.75, 0.5, 0.5, 1)
...
```{r testC7}
c7Res <- fgsea(c7, idRankedByT, nperm=np, minSize = 15, maxSize = 500, nproc = nCores) %>%
	as_tibble() %>%
	dplyr::rename(FDR = padj) %>%
	mutate(adjP = p.adjust(pval, "bonferroni")) %>%
	dplyr::select(pathway, pval, FDR, adjP, everything()) %>%
...
c7Res %>%
	...
	dplyr::select(Pathway = pathway, NES, `$p$` = pval, `$p_{adj}$` = adjP, size, sizeAtMax, Genes) %>%
	pander(split.tables = Inf,
		   caption = paste("Top", nrow(.), "enriched C7 gene-sets using a Bonferroni-adjusted p-value < 0.01 for stringency.",
		   				"The normalised enrichment score indicates which end of the ranked list appeared to be enriched, with negative values indicating the down regulated genes.",
		   				"The number of genes in the gene-set are also indicated, as are the number of genes at the point of maximum enrichment.",
		   				"The most highly ranked genes before the point of the maximal enrichment are given in the final column."),
		   style = "rmarkdown",
...
```{r plotC7_1, fig.height=4, fig.width=12, fig.cap = "*Enrichment score for the most highly ranked C7 genesets*"}
tempSet <- c7Res$pathway[1]
a <- plotEnrichment(c7[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = tempSet)
tempSet <- c7Res$pathway[2]
b <- plotEnrichment(c7[[tempSet]], idRankedByT) +
	scale_x_continuous(breaks = seq(0, length(idRankedByT), by = 2e3)) +
	labs(title = tempSet)
vpA <- viewport(0.25, 0.5, 0.5, 1)
vpB <- viewport(0.75, 0.5, 0.5, 1)
