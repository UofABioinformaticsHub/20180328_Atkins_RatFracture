...
```{r volcano-plot, fig.width=8, fig.cap = "Volcano plot with genes labelled to an FDR of 0.1. DE genes to an FDR threshold of 0.05 are highlighted in red."}
results %>%
...
```{r export-results}
write_csv(results, here::here("output/top_table.csv"))
```
...
```{r gsea-hallmark}
df <- gseaResults %>% 
  dplyr::filter(gs_cat == "H", padj < 0.01) %>% 
  mutate(
...
htmltools::tags$caption(
  htmltools::em(
    glue(
      ...
      list of genes. All p-values are Bonferroni-adjusted with p < 0.01 used for 
      significance. A Normalised Enrichment Score 
...
df %>% 
  write_csv(
    here::here("output/hallmark_gsea.csv")
  )
```
